...
def depthFirstSearch(problem: SearchProblem):
    ...
    """
    # Graph search: use visited set to avoid re-expanding the same state
    visited = set()
    # Stack stores (state, actions_to_reach_this_state)
    stack = util.Stack()
...
def uniformCostSearch(problem: SearchProblem):
    ...
    # best_cost[state] = lowest cost found so far to reach this state
    best_cost = {}
    ...
    while not pq.isEmpty():
        ...
        # Current cumulative cost (PriorityQueue can't directly give us this,
        # so we infer it from actions — if costs vary, we track cost explicitly)
...
def aStarSearch(problem: SearchProblem, heuristic=nullHeuristic):
    ...
    while not pq.isEmpty():
        ...
        for successor, action, stepCost in problem.getSuccessors(state):
            new_cost = current_cost + stepCost
            if successor not in best_cost or new_cost < best_cost[successor]:
                pq.push((successor, actions + [action]), new_cost + heuristic(successor, problem))
