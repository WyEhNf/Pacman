...
def joinFactors(factors: List[Factor]):
    ...
    # Ensure factors is a list (may be dict_values from autograder)
    factors = list(factors)

    # Compute the new variable sets
    new_unconditioned = set()
    new_conditioned = set()
    domains = factors[0].variableDomainsDict()

    for f in factors:
        new_unconditioned |= f.unconditionedVariables()
        new_conditioned |= f.conditionedVariables()

    # Any variable that appears as unconditioned cannot also be conditioned
    ...
    # Iterate over every assignment and multiply probabilities
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        # New factor: same conditioned vars, unconditioned minus the eliminated one
        ...
        # Iterate over every assignment of the ORIGINAL factor.
        # Drop the eliminated variable to get the key for the new factor,
        # then add the probability.
