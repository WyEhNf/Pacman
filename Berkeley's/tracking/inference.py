...
def constructBayesNet(gameState: hunters.GameState):
    ...
    # --- Edges ---
    # Pacman influences ghost positions; both ghost and Pacman influence
    # the noisy distance observation to that ghost.
    edges = [
        (PAC, GHOST0),
        (PAC, GHOST1),
        (PAC, OBS0),
    ...
    # Observed noisy distances: true distance ∈ [0, X_RANGE+Y_RANGE],
    # then noise adds/subtracts up to MAX_NOISE, so readings range from
    # 0 up to (X_RANGE + Y_RANGE + MAX_NOISE).
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        # Grab all CPTs, reduced by evidence
        ...
        for var in eliminationOrder:
            ...
            # Eliminate the variable if there are still unconditioned vars left
            if len(joined.unconditionedVariables()) > 1:
                factors = not_joined
                factors.append(eliminate(joined, var))
            else:
                # Only one unconditioned variable → sums to 1, discard
                factors = not_joined

        # Join all remaining factors
        result = joinFactors(factors)

        # Normalize to get a proper conditional probability
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        total = self.total()
        if total == 0:
            return  # Do nothing for empty or zero-total distributions
        for key in self:
            self[key] /= total
    ...
    def sample(self):
        ...
        for key, weight in self.items():
            ...
            if cumulative >= r:
                return key
        return None  # Fallback for empty distribution
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        """
        # Special case: ghost is in jail → only None observation is possible
        if ghostPosition == jailPosition:
            return 1.0 if noisyDistance is None else 0.0

        # Special case: observation is None but ghost is NOT in jail → impossible
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        # Build weight distribution (key by particle index to keep duplicates)
