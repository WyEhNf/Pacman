...
class QLearningAgent(ReinforcementAgent):
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        self.qVals[(state, action)] = old_q + self.alpha * (target - old_q)
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        return features * self.weights  # Counter supports dot-product via *
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        """
        features = self.featExtractor.getFeatures(state, action)
        # TD difference = (r + γ max Q(s',a')) - Q(s,a)
        old_q = self.getQValue(state, action)
        next_max = self.computeValueFromQValues(nextState)
        difference = reward + self.discount * next_max - old_q
