...
print(f'DAgger Ensemble: {len(models)} models')
...
for ep in range(20):
    ...
    while not(st.isWin() or st.isLose()) and step<500:
        g=s2g(st);t=torch.FloatTensor(g).unsqueeze(0)
...
print(f'\nDAgger Ensemble: Avg={np.mean(scores):.0f}  Wins={wins}/20  Min={min(scores):.0f}  Max={max(scores):.0f}')
