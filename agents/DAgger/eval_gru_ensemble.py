...
models=[]
for i in range(5):
    m=MD();m.load_state_dict(torch.load(os.path.join(PROJECT,f'checkpoints/gruduel_m{i}_final.pt'),map_location='cpu'))
...
for ep in range(20):
    ...
    while not(st.isWin() or st.isLose()) and step<500:
        ...
        t=torch.FloatTensor(np.stack(hist)).unsqueeze(0)
...
print(f'\nGRU+Dueling 5-model: Avg={np.mean(scores):.0f}  Wins={wins}/20  Min={min(scores):.0f}  Max={max(scores):.0f}')
