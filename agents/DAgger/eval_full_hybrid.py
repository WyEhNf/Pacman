...
from game import Directions;from pacman import GameState
...
# CNN v5 (5)
for i in range(5):
    m=CNN();m.load_state_dict(torch.load(os.path.join(PROJECT,f'checkpoints/v5_cnn_m{i}_final.pt'),map_location='cpu'))
    m.eval();cnns.append(m)
print(f'CNN v5: {len(cnns)}')

# DAgger R1 (5)
for i in range(5):
    m=CNN();m.load_state_dict(torch.load(os.path.join(PROJECT,f'checkpoints/dagger_cnn_m{i}_final.pt'),map_location='cpu'))
    m.eval();cnns.append(m)
print(f'+DAgger R1: {len(cnns)} total CNN')

# DAgger R2 (5)
for i in range(5):
    m=CNN();m.load_state_dict(torch.load(os.path.join(PROJECT,f'checkpoints/dagger2_cnn_m{i}_final.pt'),map_location='cpu'))
    m.eval();cnns.append(m)
print(f'+DAgger R2: {len(cnns)} total CNN')

# GRU+Dueling best 3
for i in [0,1,2]:
    m=GRU();m.load_state_dict(torch.load(os.path.join(PROJECT,f'checkpoints/gruduel_m{i}_final.pt'),map_location='cpu'))
    m.eval();grus.append(m)
print(f'GRU+Dueling: {len(grus)}')

print(f'\nFULL HYBRID: {len(cnns)} CNN + {len(grus)} GRU = {len(cnns)+len(grus)} models')
...
for ep in range(20):
    ...
    while not(st.isWin() or st.isLose()) and step<500:
        g=s2g(st);hist.append(g)
        if len(hist)>SEQ:hist=hist[-SEQ:]
        while len(hist)<SEQ:hist.insert(0,hist[0])

        t_cnn=torch.FloatTensor(g).unsqueeze(0)
        t_gru=torch.FloatTensor(np.stack(hist)).unsqueeze(0)

        q_cnn=sum(m(t_cnn)[0].detach().numpy() for m in cnns)/len(cnns)
        q_gru=sum(m(t_gru)[0].detach().numpy() for m in grus)/len(grus) if grus else np.zeros(5)
        if grus: q=(q_cnn+q_gru)/2
        else: q=q_cnn
...
print(f'\nFULL HYBRID: Avg={np.mean(scores):.0f}  Wins={wins}/20  Min={min(scores):.0f}  Max={max(scores):.0f}')
