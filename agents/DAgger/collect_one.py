"""Single-expert data collector. Usage:
  python scripts/collect_one.py VOID 150
  python scripts/collect_one.py ATTACK 150
"""
...
agent = AlphaBetaAgent(depth='3', evalFn='betterEvaluationFunction')
all_trajs = []
collected = 0

print(f'{tag}: {total_eps} eps, AlphaBeta d3')
t0 = time.time()
...
for ghost_tag, attack, flee, count in ghost_configs:
    for _ in range(count):
        ...
        all_trajs.append({
            ...
            'steps': len(grids), 'ghost_profile': ghost_tag, 'source': tag,
        })
        collected += 1
        ...
        if collected % 20 == 0:
            ...
            print(f'[{tag}] {collected}/{total_eps} '
                  f'recent_avg={np.mean(recent):.0f} wins={sum(1 for t in all_trajs[-20:] if t["win"])} '
                  f'ghost={ghost_tag} [{time.time()-t0:.0f}s]')

dt = time.time() - t0
scores = [t['score'] for t in all_trajs]
wins = sum(1 for t in all_trajs if t['win'])
print(f'\n[{tag}] Done: {len(all_trajs)} eps in {dt:.0f}s avg={np.mean(scores):.0f} wins={wins}')
out = os.path.join(PROJECT, 'data', f'{tag.lower()}_expert.npz')
