"""Train DQN v4 — CE-only on 370 merged eps (51% win rate). Larger model for more data."""
...
print(f'  avg={np.mean(scores):.0f}, wins={wins} ({wins/len(trajs)*100:.0f}%)')
...
for epoch in range(200):
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(model.state_dict(), os.path.join(PROJECT, 'checkpoints', 'dqn_v4_best.pt'))
...
final_path = os.path.join(PROJECT, 'checkpoints', 'dqn_v4_final.pt')
